    ...
    for i in range(0, N):
        for j in range(i + 1, N):
            ...
            if l1 == 0 and l2 == 0:
                ...
                D[jj, ii, 1, 1, 3] = D[ii, jj, 1, 3, 1]  # corrected
                ...
                D[ii, jj, 0, 1, 2] = PB * D[ii, jj, 0, 1, 1] + D[ii, jj, 1, 1, 1]  # corrected
                D[ii, jj, 1, 1, 2] = a * D[ii, jj, 0, 1, 1] + PB * D[ii, jj, 1, 1, 1] + 2. * D[ii, jj, 2, 1, 1]  # corrected
                D[ii, jj, 2, 1, 2] = a * D[ii, jj, 1, 1, 1] + PB * D[ii, jj, 2, 1, 1]
                D[ii, jj, 3, 1, 2] = a * D[ii, jj, 2, 1, 1]

                D[ii, jj, 0, 1, 3] = PB * D[ii, jj, 0, 1, 2] + D[ii, jj, 1, 1, 2]  # corrected
                D[ii, jj, 1, 1, 3] = a * D[ii, jj, 0, 1, 2] + PB * D[ii, jj, 1, 1, 2] + 2. * D[ii, jj, 2, 1, 2]  # corrected
                D[ii, jj, 2, 1, 3] = a * D[ii, jj, 1, 1, 2] + PB * D[ii, jj, 2, 1, 2] + 3. * D[ii, jj, 3, 1, 2]  # corrected
