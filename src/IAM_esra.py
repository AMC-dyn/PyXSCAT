...
Original file is located at
    https://colab.research.google.com/drive/1fLXz4nOyiiVuWqUXPi9Ts-pwoMwzbkN9
"""
...
a_H = np.array([0.493002, 0.322912, 0.140191, 0.040810]).reshape(4, 1)
b_H = np.array([10.5109, 26.1257, 3.14236, 57.7997]).reshape(4, 1)
c_H = 0.003038

a_Fminus = np.array([3.63220, 3.51057, 1.26064, 0.940706]).reshape(4, 1)
b_Fminus = np.array([5.27756, 14.7353, 0.442258, 47.3437]).reshape(4, 1)
c_Fminus = 0.653396
...
q_new = np.array([0.000000000001, 0.07070707, 0.14141414, 0.21212121, 0.28282828,
                  ...
                  6.71717172, 6.78787879, 6.85858586, 6.92929293, 7.]).reshape(1, 100)

# define the form-factors as a matrix of dimension (natoms x qpoints)
form_factor = np.zeros((2, 100))
FF_H = FF(a_H, b_H, c_H, q_new)
FF_Fminus = FF(a_Fminus, b_Fminus, c_Fminus, q_new)

form_factor[0, :] = np.asarray(FF_H)
form_factor[1, :] = np.asarray(FF_Fminus)

print(form_factor)

# geometry of HF
geometry = np.array([[0.0, 0.0, 0.917],
                     [0.0, 0.0, 0.0]])

Nq = 100  # length of q space
nAtoms = 2  # number of atoms in the fragment
...
# plot the functions
plt.plot(x, y, 'c', label='IAM for HF')
plt.xlabel("q (1/Å)")
plt.ylabel("I(q)")
