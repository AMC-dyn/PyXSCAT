Module calculate_form_factors
    ...
    subroutine obtain_i_form_factors(atoms,atom,iff_or, q1,q_or,iff)
        ...
        real(kind=dp), parameter :: pi=dacos(-1.00_dp)
        character(len=10), intent(in) :: atom
        real(kind=dp), intent(in), dimension(13)::q_or
        real(kind=dp), dimension(13):: b,c,d
        real(kind=dp), intent(in), dimension(:) :: q1
        ...
        call spline(q_or*4*pi,iff_or(index,:),b,c,d,13)
        print*,'Interpolated 3'
        do i=1,size(q1)
           iff(i)=ispline(q1(i), q_or*4*pi, iff_or(index,:), b, c, d, 13)
