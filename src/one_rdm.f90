...
enddo
    ...
    implicit none
     ...
     real(kind=dp), parameter :: phase_dbl(0:1)=(/1.d0,-1.d0/)
    ...
    real(kind=dp) :: time1, time2, cutoff
     real(kind=dp):: civ1,civ2,c,c_term,phase
...
integer(kind=ikind) newdiff,mytemp,n,count2,count_p,p,q,r,s
  ...
  real(kind=dp), parameter :: phase_dbl(0:1)=(/1.d0,-1.d0/)
  real(kind=dp) c_term,temp
  real(kind=dp) :: c1(length), c2(length)
...
!$OMP PARALLEL DO PRIVATE(ici,jci,temp,newdiff,n,mytemp,exc,nperm,samespin,myspin,idx_part,idx_hole,hole,part,tz,mylow,myhigh),&
!$OMP& PRIVATE(mya,myb,myc,sorb,porb,qorb,rorb,c_term,tz2,buffer),&
!$OMP SHARED(length2,nword,icij,phase_dbl), REDUCTION(+:SPINFREE2RDM)

do ici=1,length2
if (mod(ici,10000) .eq. 0) then
write(*,*) ici,'/',length2
endif
do jci=1,length2
...
subroutine one_rdm_two_rdm(mat,twordm,onerdm,nel)
    ...
    real(kind=dp), intent(in), dimension(:) :: twordm
    real(kind=dp), intent(out), dimension(:,:), allocatable::onerdm
    ...
    do i=1,size(mat(:,1))
      ...
      use mod_types
           ...
           real(kind=dp) :: phase, c,civ1,civ2
...
!    close(15)
      ...
      use mod_types
           ...
           real(kind=dp), parameter :: phase_dble(0:1) = (/ 1.d0, -1.d0 /)
           exc(0,1,1) = 0
           exc(0,2,1) = 0
           exc(0,1,2) = 0
           exc(0,2,2) = 0
...
!                   end if
      ...
      use mod_types
           ...
           write(*,*) 'reading ', file_read

           do i=1,numberlines
               read(15,*)ici, c1(i), c2(i), icij(1,1,i), icij(2,1,i)
           enddo
           close(15)
           write(*,*) 'read ', file_read
